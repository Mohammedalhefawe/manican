...
import 'package:manicann/features/clients/presentation/pages/archived_bookings_screen.dart';
  ...
  Widget build(BuildContext context) {
    ...
    return BlocConsumer<ClientsCubit, ClientsStates>(
      listener: (context, state) {
        if (state is GetClientCurrentBookingsErrorState) {
      ...
      builder: (context, state) {
        ...
        Widget pageContent = Padding(
          ...
          child: ConditionalBuilder(
            condition: state is! GetClientCurrentBookingsLoadingState,
            builder: (context) => Column(
              ...
              children: [
                    ...
                    myButton(
                        ...
                        onPressed: () {
                          if (cubit.detailsClient!.id != null) {
                            cubit.getClientArchivedBookings(
                                cubit.detailsClient!.id!);
                            // navigateTo(context, const ArchivedBookingScreen());
                            var appCubit = AppCubit.get(context);
                            if (AppLink.role == 'admin') {
                              appCubit.screens[4] =
                                  const ArchivedBookingScreen();
                            } else {
                              appCubit.screens[3] =
                                  const ArchivedBookingScreen();
                            }
                ...
                CustomTable<Booking>(
                  list: cubit.clientCurrentBookings,
                  type: "CurrentBookings",
                  pageIndex: cubit.currentBookingsPageIndex,
                  changeTablePage: (int newPage) =>
                      cubit.changeCurrentBookingsPage(newPage),
                  numColumns: 7,
                  labels: const [
                    'اسم الموظف',
                    'الخدمة',
